...
int comparse(int argc, char **argv) {
    ...
    if (argc < 2) {  // must have at least program t ame and len arguments
...
int main(int argc, char **argv) {
   ...
   // if (comparse(argc, argv) == EXIT_FAILURE) return showusage (EXIT_FAILURE);
    ...
    //char dBuf[7];// = {0x04,0x05,0x06,0x03,0x02,0x01,0x00};
...
// for (int j =0; j<3;j++){  
    ...
    char tBuf[] = {0x00};
    int temp = bcm2835_i2c_read(tBuf, 1);
    printf("Temperature = %i °C\n", temp);	
 fprintf(fptr,"Temperature = %i °C\n", temp);	

    const char pointBuf[] = {0x00};
    data = bcm2835_i2c_write(pointBuf,1); 

  
    const char startBuf[] = {0x00,0x00,0x00,0x61,0x01,0x03,0x01,0x01};
    slave_address = 0x68;
    bcm2835_i2c_setSlaveAddress(slave_address);
    data = bcm2835_i2c_write(startBuf,8); 
 
   
    slave_address = 0x68;
    bcm2835_i2c_setSlaveAddress(slave_address);
    bcm2835_i2c_write(pointBuf,1); 

	char tBuf1[] = {0x10};

	fclose(fptr);
    int seg, min, hrs, day, month, year, aux, j = 1;
    char date[3];
    char ap[2];
    // if (mode == MODE_READ) {
while(1){ 
...
fprintf(fptr,"Temperature = %i °C\n", temp);	
    ...
    for (i=0; i<MAX_LEN; i++) {
		...
		switch(i){
		case 0:
			seg = (((buf[i] & 0x70)/16)*10) + (buf[i] & 0x0f);
			//printf("%d segs\n",seg);
			break;
		case 1:
			min = (((buf[i] & 0x70)/16)*10) + (buf[i] & 0x0f);
			//printf("%d mins\n",min);
			break;
		case 2:
			hrs = (buf[i] & 0x10)*10 + buf[i] & 0x0F;
			ap[0] = (buf[i] & 0x20) ? 'A' : 'P';
			ap[1] = 'M';
			break;
		case 3:
			...
			break;
		case 4:
			day = (buf[i] & 0x03)*10 + buf[i] & 0x0F;
			break;
		case 5:
			month = (buf[i] & 0x01)*10 + buf[i] & 0x0F;
			break;
		case 6:
			year = (buf[i] & 0xF0)*10 + buf[i] & 0x0F;
			break;
		}

	}
    }    
    printf("Record %i: %02d/%02d/%02d %c%c%c %02d:%02d:%02d %c%c\n",j,day,month,year,date[0],date[1],date[2],hrs,min,seg,ap[0],ap[1]);
    fprintf(fptr,"Record %i: %02d/%02d/%02d %c%c%c %02d:%02d:%02d %c%c\n",j,day,month,year,date[0],date[1],date[2],hrs,min,seg,ap[0],ap[1]);
   ...
   // data = bcm2835_i2c_write(pointBuf,1);
   // slave_address = 0x4D;
   //bcm3835_i2c_setSlaveAddress(slave_address); 
   j++;
 fclose(fptr);
    ...
    for (i=0; i<MAX_LEN; i++) {
		...
		switch(i){
		case 0:
			seg = (buf[i] & 0x70)*10 + buf[i] & 0x0f;
			printf("%d segs\n",seg);
			break;
		case 1:
			min = (buf[i] & 0x70)*10 + buf[i] & 0x0f;
			printf("%d mins\n",min);
			break;
		case 3:
			day = buf[i] & 0x07;
